...
public XML[] forecast = new XML[10];
public XML location ;
public int [] low = new int[10]; 
public int [] high = new int[10]; 
public String [] weather = new String[10];
...
//Default Constructor is called
 // WOEID woeid_obj=new WOEID(); 
 
// Overridden Constructor is called
  String[] woeid_in = new String[]{"12589271","2459115","2459114"};
  WOEID woeid_obj=new WOEID(3,woeid_in); 

//

// Text Format Object
TextFormat rText=new TextFormat();

public void forcast(int frameCount)
{
    i=frameCount%woeid_obj.len;
// The URL for the XML document
  String url = "https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20weather.forecast%20where%20woeid%3D" + woeid_obj.woeid[i];
...
fill(0, 102, 153);
  ...
  textSize(15); 
  text("WOEID code: " + woeid_obj.woeid[i], 10, 100);
  fill(0);
...
for (int i=0;i<forecast.length;i++){
     ...
     fill(256,0,0);
     rText.rText(Float.toString(high[i]),50*i+50,high[i]*2 + 30);
     rText.rText(day[i],50*i+50,high[i]*2 + 50);
     //text(weather[i],50*i+50,high[i] + 70 + i*12*(i%2));
   } 
   
   line(height-250,width-200,height-200,width-200);
   rText.rText("High Temperature",height-190,width-205); 
...
for (int i=0;i<forecast.length;i++){  
   ...
   //Draw Line-Chart for Low Temp.
     ...
     rText.rText(Float.toString(low[i]),50*i+50,low[i]*2 );
   }
   
   
   line(height-250,width-220,height-200,width-220);
   rText.rText("Low Temperature",height-190,width-225); 
}
}